...
![image](https://github.com/Pure-Basket/Pure-Basket-BE/assets/35479166/a4465539-ec8d-4f7a-9c3b-34a7bb7978c0)


### 📙 ERD

![image](https://github.com/Pure-Basket/Pure-Basket-BE/assets/35479166/71b0a737-82ea-44ba-823b-14046492575d)
...
- 0000명의 유저의 조회 요청을 3초 이내로 응답

2️⃣ **주문 성능 최적화**

- 0000명의 유저의 주문 요청을 3초 이내로 응답
...
- **동시성 제어를 위해 비관적 락 적용**
    ...
    <aside>
    💡 여러 유저가 같은 상품을 주문하는 경우 발생한 동시성 문제 해결을 위해 성능 비교를 통해 DB 비관적락 적용

    </aside>

    **의사 결정 과정**

    **Lettuce을 이용한 분산락 VS Redisson을 이용한 분산락**

    - Lettuce를 이용한 분산락
        - spring-data-jpa의 기본 라이브러리를 이용해서 별도의 라이브러리 설치하지 않아도 됨
        - spin lock 방식으로 동작해 동시에 많은 쓰레드가 락을 획득하려고 대기하는 경우엔 redis 서버에 부하가 갈 수 있다.
        - retry 로직을 직접 구현해야 함
    - Redisson을 이용한 분산락
        - spring에서 사용하기 위해 별도 라이브러리 설치 필요
        - redis pub-sub 방식이라 lettuce에 비해 redis에 가해지는 부하가 적음

    ⇒ 분산락을 사용한다면 Redisson을  사용하기로 결정

    **Redis에 부하가 적은 redisson을 이용한 분산락과 비관적락을 사용한 경우 비교**

    - 10000개의 동시 요청이 들어오는 경우를 위한 test코드를 시행한 경우 Redis 분산락보다 비관적 락이 평균 1분 정도 더 빠르게 나옴.


        | Gradle 테스트 평균 속도 | redis 분산락(redisson) | 비관적락 |
        | --- | --- | --- |
        | 동시에 10000개 요청 | 5min 9sec | 3min 50sec |

- **CI/CD를 위해 Github Actions 적용**
    ...
    **GIthub Actions:** 
    ...
    - 별도의 서버 없이 Github에서 바로 실행 가능하고 초기 설정이 쉬움.
    - Jenkins에 비해 플러그인이나 레퍼런스가 적음.
    - 작은 규모의 프로젝트 또는 간단한 워크플로우에 적합

    **Jenkins:**

    - 다양한 플러그인을 지원하고 자동화 테스트를 수행함.
    - 대규모 프로젝트에서 빌드 파이프라인을 간단히 구성할 수 있음
    - 레퍼런스가 다양함.
    - 초기 설정이 복잡하고, 별도의 서버를 구성해야 하며 러닝커브가 상대적으로 가파름

- **모니터링 툴로 Pinpoint와 prometheus/grafana 사용**
    ...
    **prometheus/grafana**
    ...
    - exporter로 모니터링 대상 시스템으로부터 pull 방식으로 메트릭을 받아오는 방식으로 동작
    - 하나의 서비스만 모니터링하는 것이 아니라 연결된 다른 서비스들에 대한 모니터링
    - HA를 위한 이중화나 클러스터링이 불가능해서, thanos를 추가 설치해야함.
    - 코드 레벨로 서비스의 병목 지점을 알려주지 않음.

    **Pinpoint:** 

    - 어플리케이션의 병목 지점을 코드 레벨로 파악하여, 성능 저하 요소 및 문제 원인을 추적 가능함.
    - 분산 시스템의 구성 맵과 노드 간의 트랜잭션 수를 한 눈에 파악 가능함.
    - 특정 트랜잭션에서 실행된 메소드와 응답시간을 확인할 수 있고 어플리케이션의 오류나 예외 정보도 확인 가능
...
- **이벤트 큐로 Kafka 사용**
    ...
    | --- | --- | --- | --- |
    | 주문 내역 이벤트 보존 필요성 | 수신 확인 및 이벤트 별도 저장 | 수신 확인 | 수신 확인 없이 삭제 |
    | 다수 API의 동시 처리 가능성 | Pub/Sub 모델 | 기본적으로 단일 수신자 대상 | Pub/Sub 모델 |
...
- **Redis로 캐시 적용**
    ...
    | --- | --- | --- |
    | 사용목적 부합성
    (Refresh Token, Caching)  | 사용 목적에 부합하며 다수의 활용 사례 있음 | 사용 목적에 부합하며 AWS Elasticache에서 지원 |
    | 고가용성 | Replication 구축 가능 | Replication 불가 |
    | 사용성 | 사용 경험 있음 | 사용 경험 없음, 스터디 필요 |
- **서버 Scale out**
    ...
    |  | ALB | Nginx |
    | --- | --- | --- |
    | Scale-out 편의성 | AWS의 Auto-scaling 활용 가능
    Server 추가 시 target group에 추가로 간편하게 scale-out 가능 | 이벤트 시 수동 Scale-out 필요
    server 추가 시에 설정 파일 직접 수정 필요 |
    | 적정 리소스 할당성 | 트래픽에 따라 자원 할당 | 예상 트래픽과 다를 시 리소스 초과 또는 부족 발생 가능 |
    | HTTPS 적용 | Certificate Manger로 HTTPS 적용 가능 | HTTPS 적용을 위한 인증서 발금 및 설정 파일 직접 수정 필요 |


## 📈 06. 성능개선

- **redis 캐시로 홈페이지 조회 성능 개선**
    ...
    - Thread : 1000명 / Ramp-up time : 1 / loop count : 1
    ...
    - Redis Caching 적용 전 :  시나리오 테스트 결과 평균 응답시간 9445ms
    - Redis Caching 적용 후 :  시나리오 테스트 결과 평균 응답시간 1241ms

    1000개의 쓰레드 요청에 대한 평균 응답시간 87% 개선
...
- **카프카를 통한 주문 성능 개선**
    ...
    **카프카 적용 전/후 성능 지표**
    ...
    - ec2 - t3.2xlarge | rds - db.t3.micro
    - Thread : 1000명 / Ramp-up time : 1 / loop count : 1
    ...
    1000개의 쓰레드 요청에 대한 평균 응답시간 61% 개선

    |  | 평균 응답시간 | TPS |
    | --- | --- | --- |
    | 카프카 적용 전 | 5480ms | 32.54 |
    | 카프카 적용 후 | 2100ms | 68.1 |


    ---

    **카프카 토픽의 파티션 수에 따른 성능 비교**
    ...
    - 파티션 2개 :  시나리오 테스트 결과 평균 응답시간 2100ms
    - 파티션 3개 :  시나리오 테스트 결과 평균 응답시간 2020ms

    100개의 쓰레드 요청에 대한 응답시간 개선 미미(0.03%)
    ...
    - **Product - Image**
        ...
        ****해결 방법****
        ...
        - Fetch Join, @BatchSize, @Fetch(FetchMode.SUBSELECT) 비교
            ...
            | --- | --- | --- |
            | fetch join | 617ms | 기존 Pagination 에서 사용하던 LIMIT 구문이 등장하지 않음

            firstResult/maxResults specified with collection fetch; applying in memory     경고 로그 발생

            Fetch join 하는 image가 collection 데이터라 조회하는 데이터가 매번 달라지기 때문에 limit 구문을 사용할 수 없고 모든 데이터를 조회한 후에 memory에서 페이지 처리하기 때문에 메모리 문제 발생 가능 |
            | @BatchSize() | 652ms | 항상 @BatchSize(size = n)에서 설정한 숫자만큼의 product와 관련된 image를 조회함.

            purebasket 서버스에서는 렌딩 페이지나 레시피 페이지 등에서 보여주는 상품의 개수가 달라서, 항상 n개의 상품에 대한 image를 조회하는 것은 적합하지 않을 수도 있다. |
            | @Fetch(FetchMode.SUBSELECT) | 622ms | Subquery를 포함하면 8개의 쿼리로 @BatchSize를 사용했을 때보다 쿼리가 많음. |
        - **FetchJoin은 collection을 fetch join 하는 경우 메모리 성능 이슈가 발생할 수 있으므로 @BatchSize나 @Fetch(FetchMode.SUBSELECT를 이용하기로 함.**
...
- **DB Connection Pool의 대기 시간이 전체 응답 시간의 90% 이상을 점유**


- **Scale-up / Scale-out 해도 주문 성능 향상 안 되는 문제**

    <aside>
    💡 초당 2000개의 주문 요청에 대해 Connection Pool을 200개씩 2대로 설정하여 BE 서버의 DB Connection 대기시간은 0ms가 되었으나 비관적락이 적용된 stock 테이블 조회 쿼리의 응답시간이 3000ms 이상에서 줄어들지 않음

    </aside>

    scale up / out 성능 비교 결과 제시 → 왜 그럴까?(재고 관리는 카프카에 이벤트를 전달하기 전에 처리함. 재고 차감을 위해 비관적 락을 사용)  →  Pinpoint 확인 결과 HikariCp 의 getConnection() 메서드 실행 시간이 오래 걸림 → Connection pool 사이즈 조정하면서 test (10(default) , 50, 100, 150, 200)) → connection pool 커지면 getConnection 에 걸리는 시간은 줄지만, 락을 얻기 위해 대기하는 시간이 증가

    getConnnection() 시간 오래 걸림 → db 쓰기 속도가 좋은면 락을 해제할 때까지 걸리는 시간 줄어서 성능 향상되지 않을까? → db scale up → 성능 향상 미미함. 

    → 결국엔 락이 문제 → consumer에서 락 얻고 재고 차감하도록 로직 변경하면 성능향상 될 것같음.

    HikariCP Connection Pool size 별 TPS
...
- **Redis Cache 적용 시 SerializationException**
...
- **Kafka 적용 시 ClassNotFoundException**
    ...
    public ConsumerFactory<String, KafkaPurchaseDto> consumerFactory() {
       ...
       // configs.put(JsonDeserializer.TRUSTED_PACKAGES, "com.example.purebasketbe.*");
